...
std::unique_ptr<Pipeline> Pipeline::create_compute(
    const Device* device,
...
)
{
    ...
    vk::PipelineLayoutCreateInfo pipeline_layout_ci(
        {},
        descriptor_set_layouts.size(), descriptor_set_layouts.data(),
        0, nullptr
    );

    vk::Result r;

    std::tie(r, out->pipeline_layout) = device->logical_device.createPipelineLayout(pipeline_layout_ci);

	if (r != vk::Result::eSuccess)
	{
		sl::log_error("Failed to create compute pipline layout.");
		return nullptr;
	}
    ...
    std::tie(r, out->handle) = device->logical_device.createComputePipeline({}, pipeline_ci);

	if (r != vk::Result::eSuccess)
	{
		sl::log_error("Failed to create compute pipeline.");
		return nullptr;
	}
...
std::unique_ptr<Pipeline> Pipeline::create_graphics(
	const Device* device,
